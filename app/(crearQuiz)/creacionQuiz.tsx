  ...
  const handleFilePicker = async (
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        setFieldValue("wrong code", fileContent); // Se asigna el valor al campo wrongcode
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <Formik
        ...
        onSubmit={async (values) => {
          const exerciseObject: Tables<"exercises"> = {
            exerciseid: 0,
            authorId: authorId,
          ...
          ExerciseService.create(exerciseObject).then((response) => {
            if (response.error) {
              Alert.alert(
                "Error",
