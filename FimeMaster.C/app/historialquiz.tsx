...
type AppScreenNavigationProp = StackNavigationProp<RootStackParamList, 'HistorialQuiz'>;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.quizListContainer}>
        ...
        {quizzes.map((quiz, index) => (
          <TouchableOpacity key={index} style={styles.quizItem}>
            <View style={[styles.quizItemIcon, { backgroundColor: quiz.color }]}>
      ...
      <View style={styles.footer}>
        <TouchableOpacity onPress={() => setSelectedTab('home')}>
          <Icon name="home" size={30} color={selectedTab === 'home' ? '#000' : '#bbb'} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setSelectedTab('search')}>
          <Icon name="search" size={30} color={selectedTab === 'search' ? '#000' : '#bbb'} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setSelectedTab('stats')}>
          <Icon name="bar-chart" size={30} color={selectedTab === 'stats' ? '#000' : '#bbb'} />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setSelectedTab('account')}>
          <Icon name="account-circle" size={30} color={selectedTab === 'account' ? '#000' : '#bbb'} />
        </TouchableOpacity>
      </View>
    </View>
  );
